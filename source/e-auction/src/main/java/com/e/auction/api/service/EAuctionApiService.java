...
public class EAuctionApiService {
    ...
    public ApiMessageDto<UploadFileDto> storeFile(UploadFileForm uploadFileForm) {
        ...
        try {
            ...
            Path fileStorageLocation = Paths.get(EAuctionConstant.ROOT_DIRECTORY + typeFolder).toAbsolutePath().normalize();
    ...
    public Resource loadFileAsResource(String folder, String fileName) {
        try {
            Path fileStorageLocation = Paths.get(EAuctionConstant.ROOT_DIRECTORY + File.separator + folder).toAbsolutePath().normalize();
